...
class data_controller extends \core_customfield\data_controller {
    ...
    /**
     * Get the default value for this field in string format.  The default value is a list of valid options.
     * We just verify they exist before sending their index back.
     *
     * @return a string of comma seperated options
     */
    public function get_default_value_string() {
        $defaultvalue = $this->get_field()->get_configdata_property('defaultvalue');
        $options = field_controller::get_options_array($this->get_field());
        $defaultvaluesarray = [];
        $values = explode(",", $defaultvalue);
    ...
    public function instance_form_before_set_data(\stdClass $instance) {
        $instance->{$this->get_form_element_name()} = implode(',', $this->get_value_arr());
    }
    ...
    public function get_value() {
        if (!$this->get('id')) {
            return $this->get_default_value_string();
        }
        return $this->get($this->datafield());
    }

    public function get_value_arr() {
        if (!$this->get('id')) {
